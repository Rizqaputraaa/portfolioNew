...
interface SectionImagesClientProps {
  sections: ProjectSection[];
  projectTitle: string;
}

export default function SectionImagesClient({ sections, projectTitle }: SectionImagesClientProps) {
      ...
      <div className={styles.sectionsWrapper}>
        {sections.map((sec, idx) => (
          <section key={idx} className={styles.contentSection}>
            ...
            {/* ── Gambar (bisa diklik untuk lightbox) ── */}
            {sec.image && (
              <div
                className={`${styles.sectionImageWrap} ${styles.clickable}`}
                onClick={() => setLightbox({ src: sec.image, alt: `${projectTitle} — ${sec.name ?? `Section ${idx + 1}`}` })}
                title="Klik untuk memperbesar"
              >
                <Image
                  src={sec.image}
                  alt={`${projectTitle} section ${idx + 1}`}
                  fill
