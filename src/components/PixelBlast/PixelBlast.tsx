...
// Adapted from https://reactbits.dev/backgrounds/pixel-blast (MIT)
...
const createTouchTexture = (): TouchTexture => {
  ...
  const update = () => {
    ...
    for (let i = trail.length - 1; i >= 0; i--) {
      const p = trail[i]; const f = p.force * speed * (1 - p.age / maxAge);
  ...
  return {
    canvas, texture, addTouch, update, size,
...
const createLiquidEffect = (texture: THREE.Texture, opts?: { strength?: number; freq?: number }) => {
  const fragment = `
    ...
    void mainUv(inout vec2 uv){
      vec4 tex=texture2D(uTexture,uv); float vx=tex.r*2.0-1.0; float vy=tex.g*2.0-1.0;
      float intensity=tex.b; float wave=0.5+0.5*sin(uTime*uFreq+intensity*6.2831853);
      float amt=uStrength*intensity*wave; uv+=vec2(vx,vy)*amt; }`;
...
void main(){
  float pixelSize=uPixelSize; vec2 fragCoord=gl_FragCoord.xy-uResolution*.5;
  float aspectRatio=uResolution.x/uResolution.y;
  vec2 pixelId=floor(fragCoord/pixelSize),pixelUV=fract(fragCoord/pixelSize);
  float cellPixelSize=8.*pixelSize; vec2 cellId=floor(fragCoord/cellPixelSize);
  vec2 cellCoord=cellId*cellPixelSize,uv=cellCoord/uResolution*vec2(aspectRatio,1.);
  float base=fbm2(uv,uTime*.05)*0.5-.65;
  ...
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;i++){
      ...
      float cs=8.*pixelSize;
      vec2 cuv=(((pos-uResolution*.5-cs*.5)/uResolution))*vec2(aspectRatio,1.);
  ...
  float jitterScale=1.+(h-.5)*uPixelJitter,coverage=bw*jitterScale;
  float M;
  ...
  vec3 color=uColor;
  vec3 srgbColor=mix(color*12.92,1.055*pow(color,vec3(1./2.4))-.055,step(0.0031308,color));
  fragColor=vec4(srgbColor,M);
}`;

const PixelBlast: React.FC<PixelBlastProps> = ({
  variant = 'square', pixelSize = 3, color = '#505050', className, style,
  antialias = true, patternScale = 2, patternDensity = 1, liquid = false,
  liquidStrength = 0.1, liquidRadius = 1, pixelSizeJitter = 0,
  enableRipples = true, rippleIntensityScale = 1, rippleThickness = 0.1,
  rippleSpeed = 0.3, liquidWobbleSpeed = 4.5, autoPauseOffscreen = true,
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      transparent ? renderer.setClearAlpha(0) : renderer.setClearColor(0x000000, 1);
      const uniforms = {
      ...
      if (liquid) {
        ...
        const effectPass = new EffectPass(camera, liquidEffect); effectPass.renderToScreen = true;
      ...
      const onPointerDown = (e: PointerEvent) => {
        const { fx, fy } = mapToPixels(e); const ix = threeRef.current?.clickIx ?? 0;
        uniforms.uClickPos.value[ix].set(fx, fy); uniforms.uClickTimes.value[ix] = uniforms.uTime.value;
        if (threeRef.current) threeRef.current.clickIx = (ix + 1) % MAX_CLICKS;
      };
      const onPointerMove = (e: PointerEvent) => {
        if (!touch) return; const { fx, fy, w, h } = mapToPixels(e);
      ...
      transparent ? t.renderer.setClearAlpha(0) : t.renderer.setClearColor(0x000000, 1);
    }
